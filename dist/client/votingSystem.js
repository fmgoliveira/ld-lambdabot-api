...
exports.default = async (client) => {
    process.on('uncaughtException', (err) => {
        console.log(err);
        if (err.message === "Cannot read properties of undefined (reading 'users')")
            return;
    ...
    const getVotedMembers = async () => {
        ...
        const scarpsRes = await (0, node_fetch_1.default)('https://botlist.scarps.club/api/auth/liked/900398063607242762', {
            headers: {
                'Authorization': process.env.SCARPS_BOTLIST_TOKEN
            }
        });
        const votedScarpsList = (await scarpsRes.json().catch(err => console.log(err))).users || [];
        const labsRes = await (0, node_fetch_1.default)("https://bots.discordlabs.org/v2/bot/900398063607242762/votes");
        ...
        if (votedScarpsList)
            votedScarpsList.forEach((member) => {
    ...
    const updateMemberRoles = (votedMembers) => {
        ...
        membersWithVoted1Role.forEach((member) => {
            ...
            if (!Member || Member.amount < 1)
                member.roles.remove(process.env.VOTED1_ROLE);
        });
        membersWithVoted2Role.forEach((member) => {
            const Member = votedMembers.get(member.id);
            if (!Member || Member.amount < 2)
                member.roles.remove(process.env.VOTED2_ROLE);
        });
        membersWithVoted3Role.forEach((member) => {
            const Member = votedMembers.get(member.id);
            if (!Member || Member.amount < 3)
                member.roles.remove(process.env.VOTED3_ROLE);
        });
        membersWithVoted4Role.forEach((member) => {
            const Member = votedMembers.get(member.id);
            if (!Member || Member.amount < 4)
                member.roles.remove(process.env.VOTED4_ROLE);
        });
        votedMembers.forEach((member) => {
            ...
            if (amount >= 1) {
                try {
                    if (!user.roles.cache.has(process.env.VOTED1_ROLE))
                        user.roles.add(process.env.VOTED1_ROLE);
                }
                catch (err) {
                    console.log(err);
                }
            ...
            if (amount >= 2) {
                try {
                    if (!user.roles.cache.has(process.env.VOTED2_ROLE))
                        user.roles.add(process.env.VOTED2_ROLE);
                }
                catch (err) {
                    console.log(err);
                }
            ...
            if (amount >= 3) {
                try {
                    if (!user.roles.cache.has(process.env.VOTED3_ROLE))
                        user.roles.add(process.env.VOTED3_ROLE);
                }
                catch (err) {
                    console.log(err);
                }
            ...
            if (amount >= 4) {
                try {
                    if (!user.roles.cache.has(process.env.VOTED4_ROLE))
                        user.roles.add(process.env.VOTED4_ROLE);
                }
                catch (err) {
                    console.log(err);
                }
